...
import com.ajulay.exception.NoProjectException;
import com.ajulay.exception.NoSuchTaskException;
import com.ajulay.exception.NoTaskException;
...
public class TaskService implements ITaskService {
    ...
    public Task saveTask(final Task task) {
        if (task == null) throw new NoTaskException();
    ...
    public void changeStatus(final String taskId, final String status) throws NoSuchTaskException {
        if (taskId == null || ServiceConstant.EMPTY_VALUE.equals(taskId) ||
                status == null || ServiceConstant.EMPTY_VALUE.equals(status)) throw new NoIdException();
    ...
    public List<Task> findTaskAllByProject(final String projectId) {
        if (projectId == null || ServiceConstant.EMPTY_VALUE.equals(projectId)) throw new NoProjectException();
        return dao.findByProjectId(projectId);
    }

    @Override
    public Task findTaskById(String taskId) throws NoSuchTaskException {
        if (taskId == null || ServiceConstant.EMPTY_VALUE.equals(taskId)) throw new NoTaskException();
